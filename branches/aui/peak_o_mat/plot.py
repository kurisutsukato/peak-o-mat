...
#
# Created:     2003/11/03
# RCS-ID:      $Id: plot.py,v 1.17 2006/03/17 22:45:37 RD Exp $
# Copyright:   (c) 2002
...
try:
    ...
    _Numeric.arraytype = _Numeric.ndarray
    _Numeric.Float64 = float
    _Numeric.Int32 = int
    _Numeric.Float = float
...
class PlotCanvas(wx.Panel):
    ...
    def __init__(self, parent, **kwargs):
        ...
        self.HandCursor = wx.CursorFromImage(img)
        img = getGrabHandImage()
        img.SetOptionInt(wx.IMAGE_OPTION_CUR_HOTSPOT_X, 8)
        img.SetOptionInt(wx.IMAGE_OPTION_CUR_HOTSPOT_Y, 8)
        self.GrabHandCursor = wx.CursorFromImage(img)
        img = getMagPlusImage()
        img.SetOptionInt(wx.IMAGE_OPTION_CUR_HOTSPOT_X, 9)
        img.SetOptionInt(wx.IMAGE_OPTION_CUR_HOTSPOT_Y, 9)
        self.MagCursor = wx.CursorFromImage(img)
        ...
        self._canvasBgColour = wx.SystemSettings_GetColour(wx.SYS_COLOUR_WINDOW)
        self._bgColour = wx.Colour(240,240,250)

        self._antialias = False

    def set_antialias(self, state):
        self._antialias = state
        self.Redraw()

    antialias = property(fset=set_antialias)
    ...
    def Draw(self, graphics, xAxis = None, yAxis = None, dc = None):
        ...
        if type(xAxis) not in [type(None),list,tuple,_Numeric.arraytype]:
            raise TypeError, "xAxis should be None or (minX,maxX) "+unicode(type(xAxis))
        if type(yAxis) not in [type(None),list,tuple,_Numeric.arraytype]:
            raise TypeError, "yAxis should be None or (minY,maxY) "+unicode(type(xAxis))
    ...
    def _axisInterval(self, spec, lower, upper):
        ...
        if spec == 'none' or spec == 'min':
            ...
            raise ValueError, unicode(spec) + ': illegal axis specification'
        ...
        if self._xSpec is not 'none':
            ...
            for y, d in [(p1[1], -xTickLength), (p2[1], xTickLength)]:   # miny, maxy and tick lengths
                ...
                for x, label in xticks:
                    ...
                    if text:
                        dc.DrawText(label,pt[0]-dc.GetTextExtent(label)[0]*0.5,pt[1]+5)
        ...
        if self._ySpec is not 'none':
            ...
            for x, d in [(p1[0], -yTickLength), (p2[0], yTickLength)]:
                ...
                for y, label in yticks:
                    ...
                    if text:
                        dc.DrawText(label,pt[0]-dc.GetTextExtent(label)[0]-5,
                                    pt[1]-0.5*h)
...
def _draw4Objects():
    ...
    data1 = _Numeric.linspace(5e5, 1e6, 200000)

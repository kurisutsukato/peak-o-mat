...
class XRCModule(module.XRCModule):
    ...
    def init(self):
        ...
        self.xrc_btn_create.Bind(wx.EVT_BUTTON, self.OnLoad)
        self.xrc_btn_create.Disable()
    ...
    def _sync_controls(self):
        ...
        if plot is not None:
            ...
            if l > 0:
                ds_names = ['s{:d}'.format(x) for x in range(len(self.project[plot]))]
                self.xrc_cmb_eq_fromset.AppendItems(ds_names)
                self.xrc_cmb_eq_fromset.SetSelection(0)
                self.xrc_cmb_eq_fromset.Enable(True)
                self.xrc_cb_eq_fromset.Enable(True)

                self.xrc_cmb_bez_fromset.AppendItems(ds_names)

...
class Interactor:
    ...
    def install(self, view):
        ...
        self.view.canvas.Bind(wx.EVT_PAINT, self.OnPaint)
        self.view.canvas.Bind(wx.EVT_SIZE, self.OnSize)
    ...
    def OnMotion(self, evt):
        ...
        if self.view.state.eq('getpars') and len(self.view._cmds) > 0:
            ...
            self.view._drawRubberBand(self._mousestart, pt) # add new
            ...
            self.view._drawRubberBand(self._zoomCorner1, self._zoomCorner2) # add new
    ...
    def OnMouseLeftUp(self, evt):
        if self.view.state.eq('erase'):
            ...
            if np.any(self._mousestart != self._mousestop):
                pub.sendMessage(('plot','xmarker'), wl=self.view._xmarker)
    ...
    def OnPaint(self, evt):
        dc = wx.BufferedPaintDC(self.view.canvas, self.view._Buffer)
...
class PlotCanvas(wx.Panel):
    ...
    def _drawXMarker(self, x=None, rng=None):
        if x is not None:
            self._xmarker = x
        ...
        if self._xmarker is not None:
            if np.isscalar(self._xmarker):
                self._drawVerticalLine(self._xmarker)
            else:
                yr = self.GetYCurrentRange()
                self._drawRubberBand((self._xmarker[0],yr[0]),(self._xmarker[1],yr[1]))

    def _drawVerticalLine(self, x):
        x,_ = self.PositionUserToScreen((x,1))
    ...
    def _drawRubberBand(self, corner1, corner2):
        ...
        x0,y0 = self.PositionUserToScreen(corner1)
        x1,y1 = self.PositionUserToScreen(corner2)
        ...
        if self._xSpec is not 'none':
            ...
            for y, d in [(p1[1], -xTickLength), (p2[1], xTickLength)]:   # miny, maxy and tick lengths
                ...
                for x, label in xticks:
                    ...
                    if text:
                        dc.DrawText(label,pt[0]-dc.GetTextExtent(label)[0]*0.5,pt[1]-dc.GetTextExtent(label)[1]-10) # find something better than 25
